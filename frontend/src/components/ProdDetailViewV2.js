...
function ProdDetailViewV2({ productData }) {
  ...
  const OptionCard = ({ options, test }) => {
    return (
      <button
        className="prodOption"
        onClick={() => {
          setProdOption(options);
          setProdPrice(productData.prodPrice[test]);
        }}
      >
        {options}
      </button>
      ...
      <div className="img-and-content">
        ...
        <div className="detail-section">
          ...
          <div className="name-price">
            ...
            <span>{addComma(prodPrice)}원</span>
          </div>
          <div className="clear-both"></div>
          {/* 향 설명 박스 */}
        ...
        <div className="detail-section">
          ...
          <div>
            {productData.prodOption.map((options, index) => {
              return <OptionCard options={options} key={index} test={index} />;
            })}
            <div className="clear-both"></div>
          </div>
          {/* 수량, 구매하기 부분 */}
...
export default ProdDetailViewV2;
